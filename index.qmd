...
> I'd change this graph to polar coordinates so it looks more similar to a color wheel. The closer the point to the center, the lighter the point.

Is your proposed graphic supported by vega-lite? Why or why not? (You can provide links to documentation/stackOverflow)

> It seems like vega-lite does not have any built in handling of polar coordinates, but the first method that comes up upon researching is using Arc marks. It seems like this is what is used to create pie charts, and I think I could make areas/categories with the color points sectioned out. I would also need to make the actual arcs transparent/invisible since I only want to view the points and not the area lines.
...
viewof maxSat2 = Inputs.range([minSat2, 100], {label: "Max saturation (%)", value: 0, step: 1})
